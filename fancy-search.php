...
function Fancy_Search_install_data() {		
	...
	foreach($databaseNameSearch[0] as $databaseSearch) {
		...
		$defaut = array(
			...
			"nameField" => 's',
            "colonnesWherePost" => 'post_title, post_content, post_excerpt',
            "colonnesWherePage" => 'post_title, post_content, post_excerpt',
			"typeSearch" => "REGEXP",
			"encoding" => "utf-8",
...
//  light_box
	if( $select->typeResults == 'lightbox' || $fancysearch_type == 'lightbox'){
		...
		wp_localize_script( 'fs-lightbox', 'Lightbox_Trigger_Scroll',
			array(
